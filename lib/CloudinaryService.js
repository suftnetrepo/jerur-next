...
class CloudinaryService {
  ...
  static async uploadImage(file, { folder, resourceType = 'auto' } = {}) {
    ...
    if (file.size > maxUploadBytes) {
      throw new Error(`Image exceeds the ${Math.round(maxUploadBytes / 1024 / 1024)}MB upload limit.`);
    }
    ...
    const result = await new Promise((resolve, reject) => {
      ...
      // Next route handlers expose uploaded files as web File objects, but
      // the concrete ReadableStream implementation varies between the Node
      // and production runtimes. Converting the bounded upload to a Buffer
      // first gives Cloudinary a normal Node stream in every environment and
      // avoids Readable.fromWeb throwing before the upload starts.
      Promise.resolve(file.arrayBuffer())
        .then((arrayBuffer) => {
          const source = Readable.from(Buffer.from(arrayBuffer));
   ...
   *  - a file was selected -> uploads the replacement first, then deletes
   *    oldPublicId (if any, best-effort), and returns its
   *    { secure_url, public_id } for the caller to persist (CASE 2).
   */
  static async replaceImage(file, oldPublicId, options) {
    if (!file) return null;

    // Upload first so a transient upload failure never destroys the image
    // that is still referenced by the church record. Cleanup remains
    // best-effort after the replacement is safely available.
    const uploaded = await CloudinaryService.uploadImage(file, options);

    if (uploaded && oldPublicId && oldPublicId !== uploaded.public_id) {
